...
+ Parallel Scanvenge收集器: 复制算法 + 多线程 + 以吞吐量为目标(运行用户代码时间百分比)
...
jvm内存模型中老年代满了，继续往里添加会怎样？传统分布式系统与微服务的区别？
...
1.创建对象有哪几种方式。2比较对象有哪些方法。3.mq有什么用途。4.方法的返回值有几种。

微服务开发相关经验

手写AVL树

电话+现场面试 十个人进到教室里面 第一部分先每个人做一分钟自我介绍 不能透露姓名的那种 面试官会计时 而且自我介绍顺序是自由的 然后男同学就第一个发言了 说有工行实习经验 接着第二个同学也比较优秀 第三个同学真的是颜值超高的一个妹子 到了第四个大家突然沉默了 然后我就举手了 也不清楚这种时候讲在面试官心里是加分啊还是减分 就按部就班介绍

完了第二部分 无领导 分成两个代表方，谈判结束后有一位代表总结发言 看材料时间2分钟 讨论时间 25分钟 材料真的挺难得 两分钟不怎么看的懂 到了讨论的时候 我们这一组整体都比较内向 还有两个同学是计算机专业 一位是英语专业 试题关于金融经济的 可能由于我运气好吧 组内同学都是一脸看不懂试题的样子 我也不太懂 但还是一句一句带着大家分析谈判内容 总结了谈判内容分为三大块 第一部分大家达成一致意见 接着第二第三部分 再和对面小组谈判 

对了对面小组有一个同学真的长得特别好看 就是鹤立鸡群那种 气质谈吐都很棒 我一个女生都对妹子印象深刻 然后果不其然 面试官问问题的时候 多问了妹子两个问题 所以颜值也是实力的一部分啊 整体来说我感觉我们这组同学发言都不太积极 会影响分值

1面：手写一个多线程代码：两个线程共同打印一个数组。

数字和链表的区别 堆和栈的区别 实习经历 内存管理机制 接口

问了内存机制，string的哈希实现等，污染了我的智商，果然不应该去找虐，简单的就是集合什么的。

整体面试流程较长，从投递简历到收到offer大概过了一个多月，中间实在等不及去了别家公司上班。

结果：通过。

过程：笔试，整体时间延长，估计就在于他们要集齐一堆能够坐满食堂的人再组织一场笔试吧，整体笔试覆盖范围较大，包含数据库、架构、数据、算法、操作系统等等各方各面，语言涵盖C、java 、c#、python等等。        笔试下来是等通知，然后进行面试，中间大概有两周-三周左右。 

面试结束后就是等待HR面试聊天，红箭大概2-3周，然后会有结果或者没有结果

一二面为技术面，三面为HR。前一轮如果通过的话会让在外面等一下继续面试。技术面的面试官都很严谨，对于错误是会立刻指出的，但是也会好好和你讨论并给出一些提示和启发。

一面：1. SpringMVC流程和原理；2. Minor GC和Full GC的触发时机；3. 数据库连接池原理，了解和使用过哪些开源的连接池；4. 一道算法简单题；5. 一道系统设计，有些难度，不过面试官会启发并根据你的回答一直讨论下去并提出新问题；
