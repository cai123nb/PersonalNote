...
- 8bit 阶码: 用 8 个 bit 位来存储阶码, 这里用的是无符号值, 即最大值为 255(2^8 - 1). 但是使用`偏移量(biased)`的方式来区分正负, 默认为最大值的一半 127(255/2). 即实际的阶码值为: 阶码 - 偏移量(这里为 127).
...
- 阶码位为实际阶码位 + 127 (因为存储的值要减去 127 进行偏移), 为 6 + 127 = 133, 8 位 bit 表示为: 10000101.
