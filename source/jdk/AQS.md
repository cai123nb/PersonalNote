...
`AQS`底层源码有趣而且非常严谨, 推荐阅读`Doug Lea`关于`AQS`的论文, 了解`Doug Lea`大师在设计AQS的一些想法和思路. 通过了解`AQS`的底层设计, 可以帮助我们更好的去了解和使用`java.util.concurrent`包下面各类同步工具. 这是`JAVA`做的非常好的一点, 当别的语言还在自己造轮子的时候, `Java`已经有大师封装好了一系列的同步工具供我们使用. 需要注意一点是, 因为`AQS`是使用`FIFO`队列的方式实现, 对于优先级锁时没有办法实现的.此外篇幅有限, 有些地方并没有深入解读, 后续有时间会持续更新. 思之,慎之.
